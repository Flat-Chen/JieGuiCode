...
# }""
def get_cookie(url, head, proxy):
    proxy = {"https": "http://{}".format(proxy), "http": "http://{}".format(proxy)}
    page = requests.get(url, headers=head, proxies=proxy,
                        timeout=3)
    r = page.text
    print('text: ', r)
...
# proxy = get_Proxy()
# head = get_cookie(url, head, proxy)
