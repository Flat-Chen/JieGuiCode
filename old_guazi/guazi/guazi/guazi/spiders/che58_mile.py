...
class CarSpider(scrapy.Spider):
    ...
    def get_url_list(self):
        huachen_df = pd.read_sql("select id,url from che58_online where mileage ='表显里程'", con=self.engine_huachen)
    ...
    def start_requests(self):
        ...
        for url in url_list:
            ...
            # id=1
            yield scrapy.Request(url=start_url, headers=self.headers, meta={'id': id,'url':start_url},dont_filter=True)

    def save_data(self, id, mileage):
        sql = 'update che58_online set mileage ={} where id ={}'.format(mileage, id)
    ...
    def parse(self, response):
        if'https://m.58.com' in response.url:
            try:
                mileage = response.xpath(
                    "//h4[contains(text(),'公里')]/text()").extract_first().split(
                    '万公里')[0]
            except:
            ...
            try:
                mileage = response.xpath("//div[@class='right_lcsp']/text()").extract_first()
                mileage = re.findall(r'(\d+).(\d+)万公里', mileage)
                mileage = "{}.{}".format(mileage[0][0], mileage[0][1])
            except:
                yield scrapy.Request(url=response.meta["url"], headers=self.headers1, meta=response.meta,dont_filter=True)
            else:
                self.save_data(response.meta["id"], mileage)
