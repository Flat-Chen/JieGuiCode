...
class CarSpider(scrapy.Spider):
    ...
    def redis_tools(self, url):
        valid = redis_cli.sadd("che58:key", url)
        return valid
    ...
    def car_list_parse(self, response):
        # print(response.meta)
        car_list = response.xpath("//li[@class='item']")
        index = response.xpath("//li[@class='item']/a/@href").extract()
        if index == []:
            return
        response.meta["page"] = response.meta["page"] + 1
        for i in car_list:
            kanche = i.xpath(".//div[@class='strange-tag']/text()").extract_first()
            try:
                kanche = kanche.split("看车")[0]
            except:
                kanche = False
    ...
    def car_parse(self, response):
        ...
        if response.meta["kanche"] == False:
            car_list =[]
            car_url = response.meta["city_base"] + "{}x.shtml".format(id)
            car_list.append(car_url)
            car_list.append("https://quanguo.58.com/ershouche/{}x.shtml".format(id))
            for url in car_list:
                self.redis_tools(url)
        else:
            #  kanche_url = "https://{}.58.com/ershouche/"
            car_list = []
