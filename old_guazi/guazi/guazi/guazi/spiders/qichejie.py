...
class CarSpider(scrapy.Spider):
    name = website
    # start_urls = get_url()
    start_urls = 'http://auction.autostreets.com/onlineauction/index'
    ...
    def parse(self, response):
        ...
        page = re.findall("total : '(\d+)',", response.text)[0]
        car_list = response.xpath("//ul[@class='clearfix listtbody']")
        for car in car_list:
            url = car.xpath(".//div[@class='desc_top']/h2/a/@href").extract_first().strip()
            stat = car.xpath(".//span[@class='status']/text()").extract_first().strip()
            post_time = \
                re.findall("(\d+:\d+)", car.xpath(".//div[@class='positon-box']//span[2]/text()").extract_first())[0]
            if stat == "待开始" or stat == "拍卖中":
                continue
            else:
                yield scrapy.Request(url=url, headers=self.headers, callback=self.parse_car_list,
                                     meta={"post_time": post_time})
        if self.page <= int(page):
            url = self.next_url.format(self.page)
    ...
    def deal_price(self, id):
        ...
        status = str(text.json()["extras"]["status"])
        return sold_time, price, status
    ...
    def parse_car_list(self, response):
        ...
        try:
            ...
            item["years"] = None
            item["paytype"] = None
            item["sold_date"], item["price1"], item["status"] = self.deal_price(item["carid"])
            if item['status'] == '4':
                item['status'] = 'sale'
            elif item['status'] == '2':
                item['status'] = 'sold'
